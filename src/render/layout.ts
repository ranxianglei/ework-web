...
export function tabNavHTML(active: "projects" | "issues" | "sessions"): string {
  const p = active === "projects" ? " active" : "";
  const i = active === "issues" ? " active" : "";
  const s = active === "sessions" ? " active" : "";
  return `<nav class="tabs"><a class="tab${p}" href="/projects">项目</a><a class="tab${i}" href="/issues">Issues</a><a class="tab${s}" href="/sessions">会话</a><a class="tab" href="/settings" style="margin-left:auto">⚙️ 设置</a></nav>`;
}
