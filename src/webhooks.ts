...
// downstream consumers written against Gitea (or awork's GiteaTracker) work
// unchanged. See:
//   - awork source: src/trackers/gitea-tracker.ts (parseWebhookEvent + verifyWebhookSignature)
...
//
// We populate the fields downstream consumers actually read (see awork's
...
function recordDelivery(
  ...
  try {
    ...
    console.error("webhook: failed to record delivery:", e);
  }
}
...
async function deliver(
  ...
  console.warn(
    `webhook: gave up after ${RETRY_DELAYS_MS.length} attempts: webhook=${webhook.id} url=${webhook.url} event=${event}`
  );
}
...
export function emitIssueEvent(
  ...
  try {
    ...
    console.error(`webhook: emitIssueEvent failed (project=${projectId} issue=${issueId} action=${action}):`, e);
  }
}
...
export function emitCommentEvent(
  ...
  try {
    ...
    console.error(
      `webhook: emitCommentEvent failed (project=${projectId} issue=${issueId} comment=${commentId}):`,
      e
    );
  }
}
...
export function emitPingEvent(projectId: number, origin: string): void {
  ...
  try {
    ...
    console.error(`webhook: emitPingEvent failed (project=${projectId}):`, e);
  }
}
