...
function projectSettingsTabs(owner: string, name: string, active: "webhooks" | "members"): string {
  const base = `/${encodeURIComponent(owner)}/${encodeURIComponent(name)}/settings`;
  const w = active === "webhooks" ? " active" : "";
  const m = active === "members" ? " active" : "";
  return `<nav class="subtabs">
    <a class="subtab${w}" href="${escapeAttr(base)}/webhooks">Webhooks</a>
    <a class="subtab${m}" href="${escapeAttr(base)}/members">成员</a>
  </nav>`;
}
...
${projectSettingsTabs(project.owner, project.name, "members")}
${flashHtml}
